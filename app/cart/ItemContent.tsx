...
const ItemContent:React.FC<ItemContentProps> = ({item}) => {
  return (
    <div className="grid grid-cols-5 text-xs md:text-sm gap-4 border-t-[1.5px] border-slate-200 py-4 items-center">
        <div className="col-span-2 justofy-self-start flex gap-2 md:gap-4">
            ...
            <div className="flex flex-col justify-between">
                <Link href={`/product/${item.name}`}>{truncateText(item.name)}</Link>
        ...
        <div className="justify-self-end font-semi-bold">{formatPrice(item.price*item.quantity)}</div>
    </div>
  )
}
